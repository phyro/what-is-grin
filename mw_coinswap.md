...
1. a part of a coinswap transaction
2. spent (we spend outputs from a regular transaction in case we run out of coinswapped outputs)
...
This can be visually presented in the following way. For simplicity, let's supposed we will run the coinswap service every 3 minutes so that we only have to draw 3 blocks N, N+1 and N+2. Consider that all the inputs in these blocks were created in a coinswap transaction.
...
The above example demonstrated a coinswap working by default in the range of 3 blocks that had a few inputs and outputs. The daily coinswap service when used by default would have a much larger coinswap transaction because it would contain the range of `60*24 = 1440` blocks. This could potentially make the coinswap transaction have over a thousand inputs. We may even need to split the coinswaps into multiple transactions if the transaction was too large for a single block.
